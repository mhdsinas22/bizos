  ...
  Widget _buildBody(BuildContext context, MoneyManagementState state) {
    ...
    if (state is TransactionsLoaded) {
      ...
      var filteredList = allList.where((tx) {
        ...
        final matchesSearch = tx.personName.toLowerCase().contains(query) ||
          ...
          Expanded(
            child: filteredList.isEmpty
                ? EmptyState(
                    ...
                    actionLabel: _searchQuery.isNotEmpty || _statusFilter != 'All' ? null : 'Add Transaction',
                    onActionPressed: _searchQuery.isNotEmpty || _statusFilter != 'All'
                        ? null
                        : () => Navigator.push(
                              context,
                              MaterialPageRoute(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            GridView.count(
                              ...
                              children: [
                                _buildDetailItem('Total', CurrencyFormatter.format(tx.amount)),
                                _buildDetailItem('Paid', CurrencyFormatter.format(tx.paidAmount), color: AppTheme.success),
                                _buildDetailItem('Balance', CurrencyFormatter.format(tx.balanceAmount), color: isPending ? AppTheme.error : Colors.grey),
                              ],
                            ),
                            const SizedBox(height: 12),
                            Row(
                              ...
                              children: [
                                ...
                                if (tx.notes.isNotEmpty)
                                  Tooltip(
                                    ...
                                    child: const Row(
                                      children: [
                                        Icon(Icons.notes_outlined, size: 14, color: Colors.grey),
                                        SizedBox(width: 4),
                                        Text('Notes', style: TextStyle(fontSize: 11, color: Colors.grey)),
                                      ],
                                    ),
                                  ),
                              ],
