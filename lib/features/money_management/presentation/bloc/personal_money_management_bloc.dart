...
class PersonalMoneyManagementBloc extends Bloc<MoneyManagementEvent, MoneyManagementState> {
  ...
  PersonalMoneyManagementBloc({
    ...
    on<AddTransactionEvent>((event, emit) async {
      try {
        final createdTransaction = await addTransactionUseCase.execute(event.transaction, true);
    ...
    on<UpdateTransactionEvent>((event, emit) async {
      try {
        await updateTransactionUseCase.execute(event.transaction, true);
        if (state is TransactionsLoaded) {
          final currentList = (state as TransactionsLoaded).transactions;
    ...
    on<DeleteTransactionEvent>((event, emit) async {
      ...
      try {
        await deleteTransactionUseCase.execute(event.id, true);
